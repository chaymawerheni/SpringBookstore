...
public class CommandeRestController {
	...
	Commande createCommande(@PathVariable (value = "bookId") Long bookId, @Valid @RequestBody Commande commande) {

	return bookRepository.findById(bookId).map(book ->{return commandeRepository.save(commande);})
			.orElseThrow(() -> new ResourceNotFoundException("BookId " + bookId + " not found"));

	}

	@PutMapping("/update/{bookId}/{commandeId}")
	public Commande updateCommande(@PathVariable (value = "bookId") Long bookId, 
	...
	public ResponseEntity<?> deleteCommande(@PathVariable (value = "commandeId")Long commandeId) {
